...
lemma exercise1_2 (a b c d : ℝ) : a + b + c + d = d + (b + a) + c := by
  calc
    a+b+c+d = b+a+c+d := by rw[add_comm a]
    _= b+a+(c+d) := by rw[add_assoc (b+a)]
    _= b+a+(d+c) := by rw[add_comm c]
    _= b+a+d+c := by rw[← add_assoc (b+a)]
    _= d+(b+a)+c := by rw[add_comm (b+a)]

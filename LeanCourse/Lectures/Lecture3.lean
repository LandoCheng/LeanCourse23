...
/- We can apply these lemmas manually, or use the `rfl`/`trans` or `calc` tactics. -/
...
* Guess the name of the lemma
  ...
  - You can also press `ctrl-T` (or `cmd-T`) to search for a lemma name (choosing an option
  shows you where it is proven)

  Naming conventions:
  - a theorem named `A_of_B_of_C` establishes something of the form `A` from hypotheses of the
  form `B` and `C`.
...
* If you right-click on a definition or theorem in VS Code, the editor will show a menu with
the option to jump to the file where the theorem is proven, and you can find similar theorems
nearby.
-/
...
* We can use `apply` or `specialize` to use an universal quantification or implication in a
hypothesis. -/
...
example : x < y ↔ x ≤ y ∧ x ≠ y := by exact lt_iff_le_and_ne
...
/- A lattice is a partial order with two operations `⊔` and `⊓` that correspond to the least
upper bound and greatest lower bound of two elements in the order. -/
